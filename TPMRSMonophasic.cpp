...
TPMRSMonophasic::TPMRSMonophasic(){
...
TPMRSMonophasic::TPMRSMonophasic(int mat_id, int dimension) : TPZMatWithMem<TPMRSMonoPhasicMemory, TPZDiscontinuousGalerkin>(mat_id){
    m_dimension = dimension;
}

TPMRSMonophasic::TPMRSMonophasic(const TPMRSMonophasic & other){
...
TPMRSMonophasic & TPMRSMonophasic::operator=(const TPMRSMonophasic & other){
...
void TPMRSMonophasic::Print(std::ostream &out){
...
void TPMRSMonophasic::FillDataRequirements(TPZVec<TPZMaterialData> &datavec){
...
void TPMRSMonophasic::FillBoundaryConditionDataRequirement(int type, TPZVec<TPZMaterialData> &datavec){
    int ndata = datavec.size();
    for (int idata=0; idata < ndata ; idata++) {
        datavec[idata].SetAllRequirements(false);
        datavec[idata].fNeedsSol = true;
...
void TPMRSMonophasic::ComputeDivergenceOnMaster(TPZVec<TPZMaterialData> &datavec, TPZFMatrix<STATE> &DivergenceofPhi)
{
    int ublock = 0;
...
void TPMRSMonophasic::Contribute(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef){
...
void TPMRSMonophasic::Contribute(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ef){
...
void TPMRSMonophasic::ContributeBC(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef, TPZBndCond &bc){
        ...
        case 6 :    // Neumann BC  QN
        {
            ...
            for (int iq = 0; iq < nphi_q; iq++)
            {
                STATE qn_N = Value, qn = q[0];
                    ef(iq + first_q) += m_scale_factor * weight * gBigNumber * (qn - qn_N) * phi_qs(iq,0);
                    
                    for (int jq = 0; jq < nphi_q; jq++)
                    {
                        
                        ek(iq + first_q,jq + first_q) += m_scale_factor * weight * gBigNumber * phi_qs(jq,0) * phi_qs(iq,0);
                    }
            }
...
int TPMRSMonophasic::VariableIndex(const std::string &name) {
...
int TPMRSMonophasic::NSolutionVariables(int var) {
    switch(var) {
        case 0:
            return 1; // Scalar
        case 1:
...
void TPMRSMonophasic::Solution(TPZMaterialData &data, int var, TPZVec<REAL> &Solout){
    ...
    switch (var) {
        ...
        default:
        {
            std::cout << "TPMRSMonophasic:: Variable not implemented." << std::endl;
            DebugStop();
        }
            break;
    }
