...
void RunRKApproximation(TPMRSSimulationData * sim_data){
    ...
    switch (A) {
        case 0:{
//{-0.0004990500165,20,-2.171439886,-0.4377500117,-30.85560036,-6.25865984,0.001573640038,-0.0005245369975,1.052070031*10^(-18)}
...
TPMRSSegregatedAnalysis * CreateSFISolver(TPMRSSimulationData * sim_data){
    ...
    if (sim_data->Get_is_dual_formulation_Q()) {
        sfi_analysis->ConfigurateAnalysis(ELU, ELU, sim_data, cmesh_geomechanic, cmesh_res, mesh_vector);
    }else{
        sfi_analysis->ConfigurateAnalysis(ELU, ELDLt, sim_data, cmesh_geomechanic, cmesh_res, mesh_vector);
    }
